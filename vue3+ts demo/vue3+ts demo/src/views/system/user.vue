  ...
  <div class="content-container">
    ...
    <dataTable ref="dataTableRef" :requestApi="listUser" :tableColumn="tableColumn" :otherConfig="otherConfig"
      ...
      <template #setup="{ row, index }">
        <el-button link type="primary" size="small" v-hasPerm="['system:user:edit']" @click="handleEdit(row.id)">
          编辑</el-button>
        <el-button link type="primary" size="small" v-hasPerm="['system:user:del']" @click="handleDelete(row)">
          删除</el-button>
        <el-button link type="primary" size="small" v-hasPerm="['system:user:edit']" @click="handleReset(row.id)">重置密码
        </el-button>
      </template>
    </dataTable>
    ...
    <el-dialog v-model="dialogFormVisible" :title="title" width="25%">
      ...
      <el-form :model="form" ref="editFormRef" :rules="rules" label-width="6em">
        ...
        <el-form-item v-if="!form.id || form.action === 'edit-pwd'" label="用户密码" prop="password">
          <el-input v-model="form.password" type="password" placeholder="请输入用户密码" />
        </el-form-item>
        <el-form-item v-if="!form.id || form.action === 'edit-pwd'" label="确认密码" prop="repassword">
...
const handleDelete = (rows: userRow | Array<userRow>) => {
...
const reLogin = () => {
  ElMessageBox.confirm('修改成功，请重新登录', '重新登录', {
    ...
    userState.dispatch('resetToken').then(() => {
      location.reload()
    })
  })
}
  ...
  formEl.validate((valid, fields) => {
    if (valid) {
      ...
      if (form.action === 'edit-pwd') {
        updatePwd({ id: form.id, old_password: form.old_password, password: form.password, repassword: form.repassword }).then(res => {
          console.log(form.id, userState.state.user_id);

          if (form.id === userState.state.user_id) {
            reLogin()
          }
